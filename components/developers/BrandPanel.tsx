  ...
  useEffect(() => {
    (async () => {
      ...
      if (d.brand) {
        ...
        setAbout(d.brand.about || ""); setLogoUrl(d.brand.logoUrl || ""); setColor(d.brand.color || "#6495ED")
    ...
    <div style={S.card}>
      ...
      <div style={S.grid}>
        ...
        <label style={S.field}><span style={S.lab}>Accent colour</span>
          <div style={S.slugRow}><input type="color" value={/^#[0-9a-fA-F]{6}$/.test(color) ? color : "#6495ED"} onChange={(e) => setColor(e.target.value)} style={S.colorPick} /><input value={color} onChange={(e) => setColor(e.target.value)} style={S.input} /></div>
        </label>
...
const S: Record<string, any> = {
  ...
  muted: { fontSize: 13.5, color: "#64748B", lineHeight: 1.6, margin: "0 0 14px" },
  grid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 },
  field: { display: "flex", flexDirection: "column", gap: 5 },
  lab: { fontSize: 12, color: "#64748B", fontWeight: 600 },
  hint: { fontSize: 11.5, color: "#94A3B8", lineHeight: 1.5, marginTop: 3 },
  input: { border: "1px solid #E9EDF2", borderRadius: 8, padding: "9px 12px", fontSize: 13.5, fontFamily: "var(--font-sans)", color: "#1F2937", background: "#fff", width: "100%", boxSizing: "border-box" },
  slugRow: { display: "flex", alignItems: "stretch" },
  slugPre: { display: "flex", alignItems: "center", padding: "0 10px", background: "#F1F5F9", border: "1px solid #E9EDF2", borderRight: "none", borderRadius: "8px 0 0 8px", fontSize: 13, color: "#64748B", fontFamily: "var(--font-mono)" },
  colorPick: { width: 42, height: 38, border: "1px solid #E9EDF2", borderRadius: 8, padding: 2, background: "#fff", cursor: "pointer", marginRight: 8 },
  ...
  btn: { background: "#6495ED", color: "#fff", border: "none", borderRadius: 9, padding: "10px 18px", fontSize: 13.5, fontWeight: 600, cursor: "pointer" },
  view: { fontSize: 13, color: "#475569", textDecoration: "none" },
  link: { fontFamily: "var(--font-mono)", color: "#6495ED" },
}
