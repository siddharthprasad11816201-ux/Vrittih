...
export default function AppShell({ children, title }: { children: ReactNode; title?: string }) {
  ...
  const CoachCard = () => (
    <div style={S.coach}>
      ...
      <Link href="/career/coach" style={S.coachBtn}>Ask the coach</Link>
    </div>
  )
      ...
      <div style={S.body}>
        {!isMobile && (
          <aside style={{ ...S.sidebar, ...(rail ? S.sidebarRail : {}) }}>
            ...
            {rail
              ? <Link href="/career/coach" title="AI Career Coach" aria-label="AI Career Coach" style={S.railCoach}><IconActivity size={20} /></Link>
              : <CoachCard />}
          </aside>
        )}
