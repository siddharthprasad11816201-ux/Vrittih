...
const STATUS_COLORS: Record<string, {bg:string,color:string}> = {
  APPLIED:{bg:"#E6F1FB",color:"#2E9BE0"}, REVIEWED:{bg:"#D3EDE4",color:"#0B1126"},
...
function tier(s:number){ return s>=85?"#047857":s>=70?"#1D4ED8":s>=55?"#334EAC":s>=40?"#B45309":"#6B7280" }
function tierBg(s:number){ return s>=85?"#ECFDF5":s>=70?"#EFF6FF":s>=55?"#EAF1FE":s>=40?"#FFFBEB":"#F3F4F6" }
      ...
      <div style={{maxWidth:"1000px",margin:"0 auto",padding:"1.5rem 2rem"}}>
        ...
        {jobs.length === 0 && !loading ? (
          <div style={{background:"#fff",border:"1px solid #eee",borderRadius:"14px",padding:"3rem",textAlign:"center",color:"#6b7280"}}>
            ...
            <Link href="/dashboard/post-job" style={{color:"#6495ED",fontWeight:600}}>Post a job →</Link>
          </div>
        ) : (
          <>
            <div style={{display:"flex",gap:"12px",alignItems:"center",marginBottom:"1.25rem",flexWrap:"wrap"}}>
              ...
              {jobId && <Link href={`/jobs/${jobId}/apply/setup`} style={{marginLeft:"auto",fontSize:"13px",fontWeight:600,color:"#6495ED",textDecoration:"none",border:"1px solid #6495ED",borderRadius:"9px",padding:"7px 13px"}}>Set up application form →</Link>}
            </div>
                ...
                {candidates.map((c,i)=>{
                  ...
                  return (
                    <div key={c.userId} style={{background:"#fff",border:"1px solid #eee",borderRadius:"14px",padding:"1.1rem 1.3rem",display:"flex",gap:"14px",alignItems:"flex-start"}}>
                      ...
                      <div style={{width:"44px",height:"44px",borderRadius:"11px",background:"linear-gradient(135deg,#EAF1FE,#D3EDE4)",border:"1px solid rgba(15,110,86,.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"14px",fontWeight:700,color:"#6495ED",flexShrink:0}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:"6px",alignItems:"flex-end"}}>
                        <Link href={`/messages`} style={{fontSize:"12.5px",fontWeight:600,color:"#6495ED",whiteSpace:"nowrap"}}>Message →</Link>
                      </div>
                    </div>
                  )
                })}
