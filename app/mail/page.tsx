        ...
        <div style={S.mailList} className="mailListPane">
          <div className="mailMobileBar">
            ...
            <div style={{ display: "flex", gap: 6, overflowX: "auto" }}>
              {FOLDERS.map(f => (
                <button key={f.key} onClick={() => { setFolder(f.key); setComposing(false) }} style={{ flexShrink: 0, background: folder === f.key ? "#6495ED" : "#F1F5F9", color: folder === f.key ? "#fff" : "#475569", border: "none", borderRadius: 999, padding: "6px 14px", fontSize: 12.5, fontWeight: 600, cursor: "pointer" }}>{f.label}</button>
              ))}
            </div>
          </div>
          ...
          mails.length === 0 ? (
            ...
            <div key={m.id} onClick={() => openMail(m)} style={{...S.mailItem,...(selected?.id===m.id?S.mailItemOn:{})}}>
              <div style={{...S.mailAvatar,background:"#6495ED"}}>{initials(folder==="sent"?m.to?.name:m.from?.name)}</div>
          ...
          {selected && !composing && (
            <div style={S.readMail}>
              ...
              <div style={S.readMeta}>
                <div style={{...S.mailAvatar,width:36,height:36,fontSize:13,background:"#6495ED"}}>{initials(selected.from?.name)}</div>
                <div>
...
@media (max-width:900px){
  ...
  .mailBack{ display:inline-flex; align-items:center; gap:6px; background:#fff; border:none; border-bottom:0.5px solid rgba(0,0,0,.06); color:#6495ED; font-weight:600; font-size:13px; padding:12px 16px; cursor:pointer; width:100%; }
}
`
const S: Record<string,any> = {
  shell:{ display:"grid",gridTemplateColumns:"200px 280px 1fr",height:"calc(100vh - 60px)",overflow:"hidden",background:"#F7F9FC" },
  ...
  composeBtn:{ background:"#6495ED",color:"#fff",border:"none",borderRadius:10,padding:"10px 14px",fontSize:13,fontWeight:600,cursor:"pointer",marginBottom:8,textAlign:"left" as const },
  ...
  folderBtnOn:{ background:"#EAF1FE",color:"#6495ED",fontWeight:500 },
  badge:{ background:"#6495ED",color:"#fff",borderRadius:999,fontSize:10,fontWeight:700,padding:"1px 6px",minWidth:18,textAlign:"center" as const },
  ...
  unreadDot:{ width:7,height:7,borderRadius:"50%",background:"#6495ED",flexShrink:0 },
  ...
  sendBtn:{ background:"#6495ED",color:"#fff",border:"none",borderRadius:8,padding:"9px 20px",fontSize:13,fontWeight:500,cursor:"pointer" },
  ...
  replyBtn:{ background:"#6495ED",color:"#fff",border:"none",borderRadius:8,padding:"6px 14px",fontSize:13,fontWeight:500,cursor:"pointer" },
