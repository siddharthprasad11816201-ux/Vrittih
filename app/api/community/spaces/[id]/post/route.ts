...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (replyToId) {
      const reply = await (prisma as any).professionalSpaceReply.create({
    ...
    const post = await (prisma as any).professionalSpacePost.create({
      data: { spaceId: params.id, userId: payload.userId, content: content.trim() },
      include: {
