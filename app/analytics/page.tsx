...
export default function AnalyticsPage() {
  ...
  const STATUS_COLORS: Record<string,string> = {
    APPLIED:"#6366F1",REVIEWED:"#6495ED",SHORTLISTED:"#059669",
          ...
          <div style={S.statsGrid} data-keep-cols>
            {data.isEmployer ? (
              ...
              <>
                ...
                <div style={S.statCard}><div style={{...S.statNum,color:"#6495ED"}}>{data.responseRate||0}%</div><div style={S.statLabel}>Response rate</div></div>
                <div style={S.statCard}><div style={{...S.statNum,color:"#B45309"}}>{data.statusCounts?.INTERVIEW||0}</div><div style={S.statLabel}>Interviews</div></div>
              ...
              <div style={S.chart}>
                {(data.appsByDay||[]).map((d:any,i:number) => (
                  <div key={i} style={S.barWrap}>
                    <div style={{...S.bar,height:`${Math.max(4,(d.count/maxBarVal)*100)}%`,background:d.count>0?"#6495ED":"#E5E7EB"}} title={`${d.day}: ${d.count}`} />
                    ...
                    return (
                      <div key={f.stage} style={S.funnelRow}>
                        ...
                        <div style={S.funnelBarWrap}>
                          <div style={{...S.funnelBar,width:`${pct}%`,background:["#6495ED","#6366F1","#059669","#B45309","#16A34A","#047857"][i]}} />
                        </div>
                  ...
                  {data.jobs.map((j:any) => {
                    ...
                    return (
                      <tr key={j.id} style={S.tr}>
                        <td style={S.td}><Link href={`/jobs/${j.id}`} style={{color:"#6495ED",textDecoration:"none",fontSize:13,fontWeight:500}}>{j.title}</Link></td>
...
const S: Record<string,any> = {
  ...
  industryCard:{ background:"#EAF1FE",border:"0.5px solid rgba(15,110,86,.15)",borderRadius:10,padding:"12px",textAlign:"center" as const },
  industryCount:{ fontSize:22,fontWeight:700,color:"#6495ED" },
