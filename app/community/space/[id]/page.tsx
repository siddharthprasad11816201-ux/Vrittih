...
export default function SpacePage() {
  ...
  const colors = ["#6495ED","#059669","#0891B2","#B45309","#DC2626"]
  const getColor = (id: string) => colors[id?.charCodeAt(0)%colors.length||0]
  const roleColors: Record<string,string> = { SUPER_ADMIN:"#C026D3", ADMIN:"#6495ED", EMPLOYER:"#B45309", JOBSEEKER:"#059669" }
            ...
            <div style={S.main}>
              ...
              {!isMember && (
                <div style={S.joinPrompt}>
                  <span style={{color:"#6495ED"}}>{CATEGORY_ICONS[space.category] || <IconMessage size={20} />}</span>
                  ...
                  <div key={post.id} style={S.post}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={S.postMeta}>
                        ...
                        {post.user?.role && <span style={{fontSize:11,fontWeight:500,padding:"1px 7px",borderRadius:999,background:"rgba(15,110,86,.08)",color:roleColors[post.user.role]||"#6495ED"}}>{post.user.role}</span>}
              ...
              <div style={S.sideCard}>
                ...
                {space.members?.slice(0,10).map((m: any) => (
                  <div key={m.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                    ...
                    {m.role==="ADMIN" && <span style={{fontSize:10,color:"#6495ED",marginLeft:"auto",fontWeight:600}}>MOD</span>}
                  </div>
                ))}
              </div>
            </aside>
...
const S: Record<string,any> = {
  ...
  icon:{ width:60,height:60,display:"flex",alignItems:"center",justifyContent:"center",background:"#EAF1FE",color:"#6495ED",borderRadius:14,flexShrink:0 },
  ...
  joinBtn:{ background:"#6495ED",color:"#fff",border:"none",borderRadius:8,padding:"9px 20px",fontSize:13,fontWeight:500,cursor:"pointer" },
  ...
  postBtn:{ background:"#6495ED",color:"#fff",border:"none",borderRadius:8,padding:"7px 18px",fontSize:13,fontWeight:500,cursor:"pointer" },
  joinPrompt:{ background:"#EAF1FE",border:"0.5px solid rgba(15,110,86,.15)",borderRadius:12,padding:"1.5rem",display:"flex",flexDirection:"column" as const,alignItems:"center",gap:10,textAlign:"center" as const },
  ...
  replyBtn:{ background:"#6495ED",color:"#fff",border:"none",borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:500,cursor:"pointer",flexShrink:0 },
