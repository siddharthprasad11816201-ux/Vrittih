...
export default function SpacePage() {
  ...
  const roleColors: Record<string,string> = { SUPER_ADMIN:"#C026D3", ADMIN:"#0F6E56", EMPLOYER:"#B45309", JOBSEEKER:"#059669" }
            ...
            <div style={S.main}>
              ...
              {!isMember && (
                <div style={S.joinPrompt}>
                  <span style={{color:"#0F6E56"}}>{CATEGORY_ICONS[space.category] || <IconMessage size={20} />}</span>
                  ...
                  <div key={post.id} style={S.post}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={S.postMeta}>
                        ...
                        {post.user?.role && <span style={{fontSize:11,fontWeight:500,padding:"1px 7px",borderRadius:999,background:"rgba(15,110,86,.08)",color:roleColors[post.user.role]||"#0F6E56"}}>{post.user.role}</span>}
              ...
              <div style={S.sideCard}>
                ...
                {space.members?.slice(0,10).map((m: any) => (
                  <div key={m.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                    ...
                    {m.role==="ADMIN" && <span style={{fontSize:10,color:"#0F6E56",marginLeft:"auto",fontWeight:600}}>MOD</span>}
                  </div>
                ))}
              </div>
            </aside>
...
const S: Record<string,any> = {
  ...
  icon:{ width:60,height:60,display:"flex",alignItems:"center",justifyContent:"center",background:"#E1F5EE",color:"#0F6E56",borderRadius:14,flexShrink:0 },
  ...
  joinBtn:{ background:"#0F6E56",color:"#fff",border:"none",borderRadius:8,padding:"9px 20px",fontSize:13,fontWeight:500,cursor:"pointer" },
