...
const TYPE_META: Record<string,{label:string,color:string,bg:string,icon:React.ReactNode}> = {
  ...
  CODING:      { label:"Coding",     color:"#7C3AED", bg:"#F5F3FF", icon:<IconClipboard size={20} /> },
}
      ...
      <div style={S.page}>
        <div style={S.wrap}>
          ...
          {loading ? <div style={S.empty}><p style={{color:"#9ca3af"}}>Loading...</p></div> : (
            <div style={S.grid}>
              {tests.map(t => {
                const meta = TYPE_META[t.type] || { label:t.type,color:"#7C3AED",bg:"#F5F3FF",icon:<IconFileText size={20} /> }
                return (
...
const S: Record<string,any> = {
  ...
  startBtn:{ background:"#7C3AED",color:"#fff",padding:"10px 0",borderRadius:9,fontSize:14,fontWeight:500,textDecoration:"none",textAlign:"center" as const,marginTop:4 },
