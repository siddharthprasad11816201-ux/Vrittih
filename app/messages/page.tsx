            ...
            {conversations.map(c => {
              ...
              return (
                <div key={c.id} onClick={() => openConvo(c)} style={{...S.convoItem, ...(active?.id===c.id?S.convoItemOn:{})}}>
                  <div style={{...S.avatar, background:"#6495ED"}}>{initials(other?.name||"?")}</div>
          ...
          {!active ? (
            ...
            <>
              ...
              <div style={S.chatHead}>
                ...
                <div style={{...S.avatar, background:"#6495ED", width:38, height:38, fontSize:14}}>{initials(getOther(active)?.name||"?")}</div>
                <div>
                  ...
                  return (
                    <div key={m.id}>
                      ...
                      <div style={{...S.msgRow, justifyContent: isMe?"flex-end":"flex-start"}}>
                        {!isMe && <div style={{...S.avatar, width:28,height:28,fontSize:11,background:"#6495ED",flexShrink:0}}>{initials(m.sender?.name||"?")}</div>}
...
@media (max-width:900px){
  ...
  .msgBack{ display:inline-flex; align-items:center; justify-content:center; width:32px; height:32px; margin-right:2px; border:none; background:#F1F5F9; border-radius:8px; font-size:18px; color:#6495ED; cursor:pointer; }
}
`
const S: Record<string,any> = {
  shell: { display:"grid", gridTemplateColumns:"300px 1fr", height:"calc(100dvh - 60px)", overflow:"hidden", background:"#F7F9FC" },
  ...
  newSend: { background:"#6495ED", color:"#fff", border:"none", borderRadius:8, padding:"7px 14px", fontSize:13, cursor:"pointer" },
  ...
  bubbleMe: { background:"#6495ED", color:"#fff", borderBottomRightRadius:4 },
  ...
  sendBtn: { width:42, height:42, borderRadius:12, background:"#6495ED", border:"none", color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", flexShrink:0, transition:"background .15s" },
}