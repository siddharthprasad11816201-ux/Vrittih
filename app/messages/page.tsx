            ...
            {conversations.map(c => {
              ...
              return (
                <div key={c.id} onClick={() => openConvo(c)} style={{...S.convoItem, ...(active?.id===c.id?S.convoItemOn:{})}}>
                  <div style={{...S.avatar, background:"#7C3AED"}}>{initials(other?.name||"?")}</div>
          ...
          {!active ? (
            ...
            <>
              ...
              <div style={S.chatHead}>
                <div style={{...S.avatar, background:"#7C3AED", width:38, height:38, fontSize:14}}>{initials(getOther(active)?.name||"?")}</div>
                <div>
                  ...
                  return (
                    <div key={m.id}>
                      ...
                      <div style={{...S.msgRow, justifyContent: isMe?"flex-end":"flex-start"}}>
                        {!isMe && <div style={{...S.avatar, width:28,height:28,fontSize:11,background:"#7C3AED",flexShrink:0}}>{initials(m.sender?.name||"?")}</div>}
...
const S: Record<string,any> = {
  ...
  newSend: { background:"#7C3AED", color:"#fff", border:"none", borderRadius:8, padding:"7px 14px", fontSize:13, cursor:"pointer" },
  ...
  bubbleMe: { background:"#7C3AED", color:"#fff", borderBottomRightRadius:4 },
  ...
  sendBtn: { width:42, height:42, borderRadius:12, background:"#7C3AED", border:"none", color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", flexShrink:0, transition:"background .15s" },
}