...
- **Fixed the /profile/edit résumé upload** ("Read the file but couldn't confidently find fields"). Root cause: the in-house PDF text engine read raw bytes from text operators but never applied each font's **/ToUnicode CMap**, so subsetted / CID (Type0) fonts — what most modern exporters (Word/Canva/LaTeX) use — decoded to glyph-id gibberish, so the parser found nothing. (The endpoint already never stored the PDF; it only extracts + returns fields to review — matching the requirement.)
  ...
  - **Upgraded `lib/resume/parse.ts` toward the spec (Gen 1–2, §7–9, §20–21):** skills are now **ontology-normalized** against the in-house taxonomy (`lib/career/taxonomy` — reused, not duplicated) and classified **explicit** (Skills section) / **demonstrated** (found in experience/projects with an evidence snippet + provenance section) / **inferred** (implied via the taxonomy graph, surfaced as suggestions, never auto-added). Fixed a link false-positive ("B.Tech" → website).
  - The client already auto-fills empty profile fields (name/headline/bio/location/phone/links) from the parse, so real extraction now flows straight into the form; skills/experience/education apply with one click.
  - **Explainable evidence panel** on `/profile/edit` (spec §8/§9/§21/§41): the résumé review now shows each detected skill grouped as *listed* (explicit) / *shown* (demonstrated, with the source line as a tooltip), plus *inferred* related skills the candidate can add one-tap — the extraction is now visible and evidence-backed, not a black box.
  - **Verified:** 19/19 units — crafted Type0/ToUnicode PDF decodes correctly, standard-font/HTML/text extraction, full parse pipeline (name/email/phone/links/skills/experience/education), explicit-vs-inferred classification + evidence, website FP fixed. Build clean.
  - **Note (deeper spec generations):** evidence/knowledge graph, candidate digital twin (Phase 14), semantic matching (recruit.shortlist via the Brain), career trajectory (career coach), and explainability already exist in the platform; this hardens the ingestion foundation (Gen 1–2) they build on. No schema change.
